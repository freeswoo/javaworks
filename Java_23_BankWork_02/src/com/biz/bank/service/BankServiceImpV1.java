		...
		if(bookVO != null) {
			...
			if(intOutput > 0) {
				...
				if(intBalance >= intOutput) {
					...
					DateTimeFormatter df = DateTimeFormatter.ofPattern("yyyy-MM-dd"); // 월 부분은 대문자로
				...
				System.out.println("출금액은 0원 이상이여야 함!!");
			}
		}
		this.bookWrite(bookFile);
	}
