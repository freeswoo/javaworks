...
public class WordQuizServiceV2 extends WordQuizServiceV1 {
	...
	// 알파벳을 낱개로 분해한후 
	// 무작위로 섞어서 console에 보여주시오

	// wordVO를 현재 클래스의 모든 메서드에서 읽을수 있다
	WordVO wordVO;
	int nCount ;
	...
	public void viewEngWord() {
		...
		// nCount가 0보다 클것이고
		// 그러면 이미 생성되어있는 wordVO를
		// private view EngWord(wordVO)에게 보내서
		// 다시한번 뒤섞은 뒤 보여주기
		if(nCount > 0) {
			System.out.println();
	...
	public void quizExec() {
		...
		if(wordVO.getEngWord().equalsIgnoreCase(sc)) {
			System.out.println("------------------");
			...
			System.out.println("------------------");
			/*
			System.out.println("계속(Enter), 힌트보기(H+Enter)>> ");
